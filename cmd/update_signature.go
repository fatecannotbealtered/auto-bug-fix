...
// retryable network condition, NOT a forged-release verdict. verifySigstoreBundle
// re-types it as a retryable E_NETWORK codedError so the caller never reports it
// as the non-retryable E_INTEGRITY reserved for a genuine signature mismatch.
...
// error — is wrapped in errTrustRootUnavailable so verifySigstoreBundle can
// re-type it as a retryable network condition. None of these is a signature
// verdict: the trust metadata is a network fetch, so its failure must NOT
// collapse into E_INTEGRITY.
