...
func runReference(args []string) {
	data := map[string]any{
		...
		"exit_codes": map[string]string{
			"0":   "success",
			"1":   "generic runtime error, integrity failure, or local IO failure",
...
func referenceCommands() []map[string]any {
	return []map[string]any{
		...
		refCmd("notify", "write", "external-write", "Render and send the fixed completion card over the configured channel (default lark) from flat fields (agent-invoked at Step 8.5; no confirm token); --dry-run previews without sending", "notify_result",
			[]map[string]any{
				param("issue", "string", true, "Jira issue key"),
				param("outcome", "string", true, "auto-fix|auto-diagnose|needs-info"),
				param("to", "string", false, "recipient open_id (ou_) or chat_id (oc_); falls back to notify.target"),
				param("channel", "string", false, "notification channel (default lark); falls back to notify.channel"),
				param("mr-url", "string", false, "GitLab MR web URL (auto-fix)"),
				param("jira-url", "string", false, "Jira issue URL"),
				param("root-cause", "string", false, "问题原因"),
				param("solution", "string", false, "解决方案/诊断/待确认问题"),
				param("dry-run", "boolean", false, "render+preview without sending"),
			},
			[]string{"auto-bug-fix notify --issue PROJ-123 --outcome auto-fix --mr-url <url> --jira-url <url> --to <open_id> --compact"}),
		refCmd("update", "update", "self-update", "Update the package and sync the Skill in one call (no confirm token); --check/--dry-run are optional read-only", "update_result",
			[]map[string]any{
				param("check", "boolean", false, "read-only update check"),
				param("dry-run", "boolean", false, "read-only preview of the update plan (no confirm token)"),
				param("target-version", "semver", false, "specific version"),
			},
			[]string{"auto-bug-fix update --check --compact", "auto-bug-fix update --dry-run --compact", "auto-bug-fix update --compact"}),
...
func referenceSchemas() map[string]any {
	return map[string]any{
		...
		"notify_result":    schema("object", []string{"sent", "messageId", "channel", "recipient", "outcome", "wouldSend", "preview"}, nil),
