...
func referenceCommands() []map[string]any {
	return []map[string]any{
		...
		refCmd("notify", "write", "external-write", "Render and send the fixed Lark completion card from flat fields (agent-invoked at Step 8.5; no confirm token); --dry-run previews without sending", "notify_result",
			[]map[string]any{
...
func referenceSchemas() map[string]any {
	return map[string]any{
		...
		"notify_result":    schema("object", []string{"sent", "messageId", "chatId", "recipient", "outcome", "wouldSend", "preview"}, nil),
