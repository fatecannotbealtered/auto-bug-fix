...
// symbols and stays identical across tools. This repo's main package is the
// module root (main.go at root, cmd/ is a library), so the entrypoint is `.`.
func fccReferenceFacts(t *testing.T) ([]string, string) {
	t.Helper()
	cmd := exec.Command("go", "run", ".", "reference", "--format", "json")
	...
	walk = func(item map[string]any) {
		...
		if len(children) == 0 {
			...
			id = strings.TrimSpace(strings.TrimPrefix(strings.TrimSpace(id), "auto-bug-fix"))
			if id != "" {
...
// reference path (e.g. <issueKey>) are stripped before matching.
