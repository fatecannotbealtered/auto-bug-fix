...
- **Workflow reads the full ticket and bases the fix on the branch the ticket names.** The agent now reads the Jira `description` (not just the summary) — which carries repro steps, environment, the affected repo, and an explicit **development branch** — and bases both the work branch and the MR target on that branch (falling back to `default_branch` only when the ticket names none). Previously, working off the summary alone could branch off and target the wrong base branch.
