...
Run through this gate **before the first public push** of `auto-bug-fix`. It is a security and quality checkpoint, not documentation — every box must be ticked (or consciously waived with a note) before the repo goes public. Once public, secrets in history cannot be un-leaked.
...
- [ ] `.env`, `*.local`, credential files, and `~/.auto-bug-fix/` artifacts are listed in `.gitignore` and confirmed untracked (`git status --ignored`).
...
- [ ] `LICENSE` is present, the license is chosen deliberately (default MIT), and `2026` / `guosong6886@gmail.com` are filled in.
- [ ] Install blocks are copy-paste-runnable and use the real published `@fateforge/auto-bug-fix` / `fatecannotbealtered/auto-bug-fix`.
...
- [ ] If `auto-bug-fix` wraps a third-party product (Jira Data Center, GitLab, Kibana), `NOTICE.md` carries the trademark / non-affiliation notice and `docs/COMPATIBILITY.md` lists the verified backend version matrix.
...
- [ ] `skills/auto-bug-fix/SKILL.md` is present; frontmatter includes `version`, `license: MIT`, `user-invocable: true`, `metadata.requires.bins`, and `metadata.requires.min_version` matching the CLI version.
...
- [ ] `skills/auto-bug-fix/test-prompts.json` is present, valid JSON, and covers fresh-agent read, write safety or read-only boundary, permission boundary, `_untrusted` handling, and self-update.
- [ ] `update --confirm` syncs the whole `skills/auto-bug-fix/` directory or returns a `skill_sync_command` equivalent to `npx skills add fatecannotbealtered/auto-bug-fix -y -g`.
- [ ] `auto-bug-fix reference`, `auto-bug-fix context`, and `auto-bug-fix doctor` run and emit valid JSON envelopes — an agent can self-onboard from a clean checkout.
- [ ] `auto-bug-fix reference` exposes `release_readiness`, and `auto-bug-fix doctor` reports the matching check.
