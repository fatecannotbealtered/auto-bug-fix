  ...
  for (const name of fs.readdirSync(skillsDir)) {
    ...
    descriptors.push({
      ...
      read() {
        ...
        const min = text.match(/(?:^|\s)min_version:\s*"?([^"\n]+)"?/m) || text.match(/"min_version"\s*:\s*"([^"]+)"/);
      ...
      write(version) {
        ...
        let next = text.replace(/^(version:\s*)"?[^"\n]+"?\s*$/m, `$1${version}`);
        next = next.replace(/(^\s*min_version:\s*)"?[^"\n]+"?\s*$/m, `$1${version}`);
...
// Go default version literal in cmd/cmd.go (`var version = "x.y.z"`). It is
// overridden at release via -ldflags, but the committed literal must still track
// package.json so non-release builds (go run/go build) and the CI guard never
// drift from the single source of truth.
function goVersion(root) {
  const file = path.join(root, "cmd", "cmd.go");
  if (!fs.existsSync(file)) return null;
  const re = /(var version = ")([^"]+)(")/;
  return {
    label: "cmd/cmd.go version",
    read() {
      const m = readRaw(file).match(re);
      return m ? [{ where: "var version", value: m[2] }] : [];
    },
    write(version) {
      const t = readRaw(file);
      const next = t.replace(re, `$1${version}$3`);
      return writeTextPreserving(file, t, next);
    },
  };
}
