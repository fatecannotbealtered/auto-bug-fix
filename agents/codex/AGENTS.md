...
- A dedicated **`jira-cli` / `gitlab-cli` / `kibana-cli` skill is installed in your skills directory** (`$CODEX_HOME/skills/<name>/SKILL.md`) — load the matching skill (e.g. `/use jira-cli`) and follow its exact subcommands and flags before driving that CLI; never guess a flag from memory.
